...
def extract_text(image_file_path, hi_lo_file_path):
    ...
    ite = 30
    t_text, w_text, h_text, hi_text, lo_text, b_text = None, None, None, None, None, None
    ...
    while ite > 0:
        ...
        print(f"Temp: {t_text}, Windchill: {w_text}, Hum: {h_text}, High: {hi_text}, Low: {lo_text}, Baro: {b_text}")
    ...
    return (f"Skyview Weather Today: {t_text} deg, Wind Chill {w_text} deg, Humidity {h_text} perc", float(t_text), float(w_text), float(h_text), float(hi_text), float(lo_text), float(b_text))
...
def main():
    ...
    while True:
        ...
        try:
            ...
            insert_weather_data(temp, wind, hum, hi, lo, baro, db_name, db_user, db_password, db_host, db_port)
            create_daily_table(db_name, db_user, db_password, db_host, db_port) 
            insert_daily_data(temp, wind, hum, baro, db_name, db_user, db_password, db_host, db_port)
